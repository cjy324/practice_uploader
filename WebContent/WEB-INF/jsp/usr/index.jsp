	...
	function startUpload(e){
	    ...
	    for(let i = 0; i < newFileList.length; i++){	
	    	...
	    	if(slicedFiles.length !== 0){
	    		// 로컬스토리지에 분할파일 배열의 길이 저장
	    		window.sessionStorage.setItem('slicedFilesLength', slicedFiles.length);
	    		...
	    		for(let k = 0; k < slicedFiles.length; k++){
				    ...
				    xhttp.open("POST", "/", true); // 메서드와 주소 설정
				    // http 요청
		        ...
		        formData.append("files", newFileList[i]);
		     	// http 요청 타입 / 주소 / 동기식 여부 설정
			    xhttp.open("POST", "upload.jsp", true); // 메서드와 주소 설정
			    // http 요청
			    xhttp.send(formData);   // 요청 전송(formData 전송)
			 	// XmlHttpRequest의 요청
			    xhttp.onreadystatechange = function(e){   // 요청에 대한 콜백
